...
# end = 100*end
for i in range(10):
    res = smacofSphere.smacofSphere(end, itmax = 5000, init="random")
...
for i in range(len(best_coords)):
    ...
    if(gmap_output):
        ...
        pos_vals3d[Gc.nodes(data=True)[i][0]] = str(100*best_coords[i][0])+","+str(100*best_coords[i][1])+","+str(100*coords[i][2])#str(coords[i][0])+","+str(coords[i][1])+","+str(coords[i][2])
        ...
        pos_vals3d[Gc.nodes(data=True)[i][0]] = str(best_coords[i][0])+","+str(best_coords[i][1])+","+str(coords[i][2])#str(coords[i][0])+","+str(coords[i][1])+","+str(coords[i][2])
